...
final class APIManager: NSObject {
    ...
    static func request(_APIRequest: APIRequest, succeed: @escaping (_ resultJSON : Any, _ message : String?) -> Void, failed: @escaping (_ message : String) -> Void) {
        ...
        print("***********************************")
        print("Headers: ", _APIRequest.headers!)
        print("End point: ", _APIRequest.endPoint!)
        print("URL Parameters: ", _APIRequest.urlParameters ?? nil)
        print("Body Parameters: ", _APIRequest.bodyParameters ?? nil)
        print("***********************************")
        
        if (_APIRequest.httpMethod! == HTTPMethod.get){
            AF.request(_APIRequest.endPoint, method: .get, parameters: _APIRequest.bodyParameters ?? nil, encoding: JSONEncoding(), headers: nil).responseJSON { response in
